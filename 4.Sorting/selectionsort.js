function selectionSort(arr) {
    ...
    for(let i = 0; i < n - 1; i++) { // i < n-1 means we just want to go till second last element
        ...
        if(min_idx != i) {
            // min element and the first element of unsorted regions are diff, then swap
            let temp = arr[i];
            arr[i] = arr[min_idx];
            arr[min_idx] = temp;
        }
...
/**
 * This function finds out the index of the minimum element in the array from
 * index [i, n-1]
 */
function findMinElement(arr, i) {
    ...
    for(let j = i+1; j < arr.length; j++) {
        if(arr[j] < arr[min_idx_el]) {
            // if the current element is smaller than the assumed min element, we update the assumed
            // min element
            min_idx_el = j;
        }
    }
